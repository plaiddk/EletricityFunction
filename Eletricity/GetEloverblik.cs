...
namespace Eletricity
{
    ...
    public class GetEloverblik
    {
        ...
        private readonly Prognose _prognose;


        public GetEloverblik(IOptions<ELOverblikSettings> eloverblikAccess, ElOverblikToken eloverblikToken, Prices prices, Metering metering, Spotprices spotPrices, Tariff tariff, Prognose prognose)
        {
      ...
      public  Task<string> Spot(
        ...
        [FunctionName("GetPrognoseData")]
         ...
         //public  async Task RunAsync([TimerTrigger("0 */5 * * * *")]TimerInfo myTimer, ILogger log)
      public  Task<string> GetPrognoseData(
          [HttpTrigger(AuthorizationLevel.Function, "get", Route = null)] HttpRequest req, ILogger log)
#else
        public Task<string> GetPrognoseData(
         [HttpTrigger(AuthorizationLevel.Function, "get", Route = null)] HttpRequest req, ILogger log)
